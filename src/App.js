...
const DEFAULT_BASE_URL = 'https://litellm.plat-eng.prod.cloud.siriusxm.com';
const ACCEPTED_FORMATS = ['.mp3', '.wav', '.mp4', '.m4a', '.flac', '.ogg', '.webm', '.aac'];
...
const AudioTranscriptionApp = () => {
  ...
  const [apiKey, setApiKey] = useState(process.env.REACT_APP_LITELLM_API_KEY || '');
  const [availableModels, setAvailableModels] = useState([]);
  const [checkingModels, setCheckingModels] = useState(false);
  const baseUrl = process.env.REACT_APP_LITELLM_BASE_URL || DEFAULT_BASE_URL;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Advanced Configuration - At Bottom */}
        <div className="bg-white rounded-lg shadow-md p-6">
          <h2 className="text-xl font-semibold mb-4">Advanced Configuration</h2>

          <div className="mb-4 p-4 bg-amber-50 border border-amber-200 rounded-lg">
            <h3 className="text-sm font-semibold text-amber-900 mb-2">Need a LiteLLM API key?</h3>
            <p className="text-sm text-amber-800">
              For security, each teammate should use their own key instead of sharing one.
            </p>
            <ol className="text-sm text-amber-800 mt-2 list-decimal list-inside space-y-1">
              <li>Open the internal LiteLLM Key Manager dashboard.</li>
              <li>Create a new key for yourself/team usage.</li>
              <li>Paste that key into the field below.</li>
            </ol>
            <a
              href="https://litellm-key-manager.infra-eng.prod.cloud.siriusxm.com/dashboard"
              target="_blank"
              rel="noreferrer"
              className="inline-block mt-3 text-sm font-medium text-amber-900 underline hover:text-amber-700"
            >
              Open LiteLLM Key Manager
            </a>
          </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="Enter API key or use REACT_APP_LITELLM_API_KEY"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-gray-50"
                />
              </div>
              <p className="text-xs text-gray-500 mt-1">
                For local setup, add REACT_APP_LITELLM_API_KEY to your .env file.
              </p>
            </div>
          </div>
